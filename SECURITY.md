...
A one-shot static security audit (GLM-5.3, 2026-08-16) reviewed 12 key files and
...
- **PRA-007 — Permissive compliance mode (`broker.py`).**
...
- **Plaintext credential storage (`config.py`) — unnumbered in the report.**
...
High/medium findings from the same audit are addressed in dedicated PRs
(PRA-001, -002, -005, -008, -015). Informational findings are tracked as
GitHub issues.
