...
- **Version:** `0.26.1` (`hybridagent/__init__.py`; `pyproject.toml` reads it dynamically)
...
- **Phase 4 implementation:** `0.26.1` checkpoint substrate in progress — tenant/workspace-owned runs, strict JSON state/schema manifests, immutable append-only checkpoint ancestry, typed interrupts, resume compatibility, cancellation, restart durability, atomic immutable effect receipts with fingerprinted idempotent replay, and scoped fork/replay lineage that never inherits prior effects; 13 focused contracts green.
