...
- **Version:** `0.28.19` (`hybridagent/__init__.py`; `pyproject.toml` reads it dynamically)
- **Active branch:** `main` (consolidation + vertical build-out + law-firm pack merged locally; pending push)
...
- **Current WIP:** None — Active Memory Consolidation (Phase 6) + Forensic/Legal vertical build-out (Phase 7) + Law Firm pack (Phase 8) all `passing`.
- **Phase 6 status:** `0.28.7` — Active Memory Consolidation complete. 6 slices (schema → consolidator → daemon wiring → metadata-on-ingest → CLI+dashboard → bug-hunt+dogfood+harden+flip). 59 consolidation tests, full suite green, 40/40 evals, ruff/mypy clean. Bug-hunt (22 hostile probes) found + fixed 1 real bug (`max_connections=0` off-by-one). Dogfood against Spark Qwen3.6-35B-NVFP4 surfaced insight-quality issue (reasoning-model CoT leaking into stored insights); fixed with prompt tightening + `_strip_reasoning` post-filter + `max_tokens=4096` for the insight pass. Reasoning-model provider support added (`content`-null → `reasoning` fallback). Default flipped on by Michael's ship decision. Released as `v0.28.7` (pushed to origin + GitHub Release).
- **Phase 7 status:** `0.28.14` — Forensic Engineering / Law Firm vertical build-out complete. 7 gaps from the 13-state regulatory gap analysis built per the gap analysis Part 4 build order: Gap 1 (per-jurisdiction registry — 13 states, forensic + legal profiles, confidence tracking), Gap 3 (MA 201 CMR 17.00 WISP + encryption attestation surface, 3 tiers), Gap 2 (NY 22 NYCRR 1200 + FL attorney-advertising filing workflow), Gap 6 (matter-wide legal hold + custodian acknowledgment), Gap 4 (per-state CE/PDH credential tracking), Gap 5 (conflict-of-interest checking across matters with no-leak + break-glass audit), Gaps 7-8 (privilege log + expert-witness disclosure templates). 7 new modules (~1,800 lines), 7 test files (224 tests). Every module sources per-state rules from the Gap 1 registry; all standalone (reference artifacts by id, no canonical-IR touch, no governance-spine changes). 2 bugs caught by tests during development (MD CLE flag, credentials session-history loss) — both fixed. Full suite green, 40/40 evals, ruff/mypy clean (156 source files), architecture invariants pass. Released as `v0.28.14` (pushed to origin + GitHub Release).
- **Phase 8 status:** `0.28.19` — Law Firm pack complete. 5 slices (manifest+persona+knowledge → skills → vertical eval suite → dashboard surfaces → 13-state integration test). The bundled, public, MIT-licensed `law_firm` pack assembles the Phase 7 compliance modules into a single one-activation experience covering the 13 states: pack.json (persona with UPL+IOLTA+per-jurisdiction guardrails, regulated risk policy holding SEND/DESTRUCTIVE, 13-tool allowlist, 4 skills, knowledge base, navy theme), 4 skills (conflict-check, ad-filing-gate, matter-hold, ce-status) wrapping the v0.28.14 modules, vertical eval suite (VerticalSpec + 5 manual cases: UPL guardrail, NY ad-filing gate, MA WISP attestation, conflict check, CLE status), dashboard surfaces (matter-hold badge, credential card, ad-filing tracker, attestation panel via /api/law_firm), 13-state integration test (83 cases proving per-jurisdiction behavior for all 13 states). 17/17 vertical evals. Bug caught by the integration test: MD cle_required=True but cle_hours=0 made it behave like MA's no-requirement — fixed with a placeholder 12/yr pending verification. Pending: tag + push to origin.
