...
- **Version:** `0.27.8` (`hybridagent/__init__.py`; `pyproject.toml` reads it dynamically)
...
- **Current blocker:** Exact-SHA review invalidated Phase 5 candidate `8cb9a360ff019a976450ba4b8d251103f22704c9` after reproducing duplicate-member JSON acceptance, lone-surrogate identity failures, and malformed signature-only image bytes reaching durable version admission. The replacement remediation rejects those payloads before canonical identity or persistence and passes the expanded pre-freeze matrix; a new immutable candidate still requires exact-commit release gates and three independent exact-SHA PASS reviews before PP50 promotion or any branch push.
...
- **Phase 5 local verification:** The current `0.27.8` remediation passes the expanded PP50 focused gate (76/76), 1,295 non-fuzz tests with 13 expected skips, 11 parser fuzz tests, 40/40 capability evaluations, the governed semantic demo, 25/25 repeated cross-process version/release races, whole-repository Ruff, mypy across 135 modules, architecture checks, compilation, added-line secret scanning, and diff hygiene. Exact regressions reject duplicate JSON members, lone Unicode surrogates, model subclasses at canonical identity, truncated/signature-only/empty-scan/scan-before-frame media, malformed/prefixed SVG, alternate-key append-only version/release replacement via `INSERT OR REPLACE`, and malformed assets before any document, version, or asset row is persisted. Candidates `8cb9a360…` and `6381f06…` and all evidence bound to them are superseded by their review rejections. Exact-commit coverage, portability, package/install, Docker, and three independent review gates remain required on the replacement candidate before promotion.
