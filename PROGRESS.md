...
- **Version:** `0.27.3` (`hybridagent/__init__.py`; `pyproject.toml` reads it dynamically)
...
- **Phase 5 local verification:** PP50 focused gate: 48 passed. Hermetic non-fuzz suite: 1,273 passed, 13 skipped, 83% coverage. Parser fuzz: 11 passed. Evals: 40/40. Whole-tree Ruff, mypy across 135 modules, architecture, compilation, semantic governed demo, and diff checks pass. Wheel/sdist build, Twine, recursive Artifact Studio wheel contents, neutral clean-wheel import, standard installer/demo, and Docker 0.27.3 status/dashboard/static-asset health pass. Twenty-five repeated version/release runs and dedicated cross-process expected-head/idempotency races pass. Added-line scan found zero hardcoded secrets, shell execution, dynamic eval/exec, unsafe pickle, or SQL-fstring patterns across 2,753 lines.
