...
- **Version:** `0.27.5` (`hybridagent/__init__.py`; `pyproject.toml` reads it dynamically)
...
- **Current blocker:** Phase 5 remediation is in verification. A late adversarial review invalidated the prior candidate after exposing release identity/media binding, canonical ZIP verification, portable Office-member, and duplicate-occurrence comparison gaps. A new exact candidate still requires the complete local release-quality matrix and three independent PASS reviews before PP50 promotion and the single final branch push.
...
- **Phase 5 implementation:** Artifact Studio now provides strict canonical professional-document models; exact evidence/source/span/claim references; deterministic JSON/Markdown and optional DOCX/PDF/PPTX/XLSX renderers; append-only scoped versions and assets; expected-head CAS; occurrence-preserving semantic comparison; exact professional-release review/signature binding; active signer/role revalidation; durable scoped idempotency; and deterministic self-verifying release bundles containing selected renders, assets, claims/evidence, reviews/signatures, validation, and run/checkpoint provenance. SQLite blocks update, delete, conflict-upsert, and replacement mutation paths. Standalone verification rejects malformed or noncanonical manifests/payloads and ZIP metadata/order, traversal, drive-qualified or Windows-reserved paths, duplicate/case-colliding names, symlinks, size violations, unexpected members, media mismatches, unreadable payloads, and release/document identity tampering.
- **Phase 5 local verification:** The `0.27.5` remediation passes the PP50 focused gate (49/49), the hermetic non-fuzz suite (1,278 passed, 13 skipped at 83% coverage), parser fuzz (11/11), capability evals (40/40), whole-tree Ruff, mypy across 135 modules, architecture checks, compilation, the semantic governed demo, and diff hygiene. Wheel/sdist build, Twine, recursive Artifact Studio wheel contents, neutral clean-wheel import, standard installer/demo, and rebuilt Docker `0.27.5` status/dashboard/static-asset health pass. Twenty-five repeated version/release runs and dedicated cross-process expected-head/idempotency races pass. Adversarial regressions now cover release/document identity and exact media binding, canonical ZIP order/metadata and unreadable members, Windows-reserved/drive-qualified/case-colliding package paths, and duplicate-identifier semantic comparison. The exact-SHA reviews for prior `0.27.4` candidates are invalidated; three fresh PASS reviews remain required before PP50 promotion.
