...
- **Phase 5 local verification:** The `0.27.5` remediation passes the PP50 focused gate (49/49), the hermetic non-fuzz suite (1,278 passed, 13 skipped), parser fuzz (11/11), capability evals (40/40), whole-tree Ruff, mypy across 135 modules, architecture checks, compilation, the semantic governed demo, and diff hygiene. Adversarial regressions now cover release/document identity and exact media binding, canonical ZIP order/metadata and unreadable members, Windows-reserved/drive-qualified/case-colliding package paths, and duplicate-identifier semantic comparison. Wheel/sdist and neutral clean-install validation will be rerun from the frozen remediation commit before fresh exact-SHA reviews. The prior `0.27.4` candidate and its verdicts are invalidated.
