...
- **Version:** `0.27.4` (`hybridagent/__init__.py`; `pyproject.toml` reads it dynamically)
...
- **Current blocker:** None. Phase 5 implementation and the local release-quality matrix pass; the exact candidate still requires three independent PASS reviews before PP50 promotion and the single final branch push.
...
- **Phase 5 implementation:** Artifact Studio now provides strict canonical professional-document models; exact evidence/source/span/claim references; deterministic JSON/Markdown and optional DOCX/PDF/PPTX/XLSX renderers; append-only scoped versions and assets; expected-head CAS; semantic comparison; exact professional-release review/signature binding; active signer/role revalidation; durable scoped idempotency; and deterministic self-verifying release bundles containing selected renders, assets, claims/evidence, reviews/signatures, validation, and run/checkpoint provenance. SQLite blocks update, delete, conflict-upsert, and replacement mutation paths. Standalone verification rejects malformed or noncanonical manifests/payloads, traversal, duplicate/case-colliding names, symlinks, size violations, unexpected renders, and hash/identity tampering.
- **Phase 5 local verification:** PP50 focused gate: 49 passed. Hermetic non-fuzz suite: 1,274 passed, 13 skipped, 83% coverage. Parser fuzz: 11 passed. Evals: 40/40. Whole-tree Ruff, mypy across 135 modules, architecture, compilation, semantic governed demo, and diff checks pass. Wheel/sdist build, Twine, recursive Artifact Studio wheel contents, neutral clean-wheel import, standard installer/demo, and Docker 0.27.4 status/dashboard/static-asset health pass. Twenty-five repeated version/release runs and dedicated cross-process expected-head/idempotency races pass. Full-branch scan found zero hardcoded secrets, shell execution, dynamic eval/exec, unsafe pickle, or SQL-fstring patterns across 5,373 added lines. The first exact-SHA review rejected `4c099e2d0f131eaad422b823a889dc9142d25645`; 0.27.4 adds regressions and remediation for artifact identity/order/container semantic comparison, strict canonical embedded-document verification, and source-checkout optional-backend guidance. Three fresh exact-head PASS reviews remain required.
