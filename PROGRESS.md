...
- **Version:** `0.26.7` (`hybridagent/__init__.py`; `pyproject.toml` reads it dynamically)
...
- **Current blocker:** independent exact-head maker-checker review and remote promotion for `0.26.7`; PP40 remains `in_progress` until both pass.
- **Phase 4 implementation:** `0.26.7` expands the checkpoint substrate from GoalRunner into planner-driven professional workflows — durable `PlanExecutor` state mapping; expected-head checkpoint CAS; scoped checkpoints on step transitions; terminal-run and pre-step cancellation enforcement; typed approval interrupts; exact durable approval reconstruction; resumable held, running, and skipped-dependent `PlanStep` state; durable replan generation/budget state; durable consequential outbox intents; exact action/receipt fingerprint binding; immutable effect receipts; explicit provider-idempotent at-least-once crash-gap semantics; daemon task-action outbox claims and receipts; multi-action completion and rejection reconciliation; exact counted restored grants; truthful manual-reconciliation outcomes; typed workspace-scoped professional reviews with maker-checker, role, active-user, database immutability, and concurrent-decision enforcement; and a structured research supervisor with active-actor reads/writes, bound compatible review snapshots, atomic review interruption/application, and decision-specific outcomes.
- **Phase 4 local verification:** `0.26.7` passes 1,195 non-fuzz tests with 17 expected skips; 11 parser fuzz tests; 114 PP40 contracts; 40/40 capability evals; 81.94% coverage; Ruff; mypy across 121 modules; architecture; semantic demo; populated `v0.25.20` migration; wheel/sdist, `twine check`, clean-wheel install, standard installer, and rebuilt Docker dashboard smoke. Independent exact-head review remains pending.
